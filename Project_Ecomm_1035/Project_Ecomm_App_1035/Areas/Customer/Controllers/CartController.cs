        ...
        public IActionResult SummaryPost(string stripeToken,string ids)
         {
            ...
            //these bellow code remove the cheack box when summary of the product comes.
            //ShoppingCartVM.orderHeader.ApplicationUser = _unitofwork.ApplicationUser.FirstOrDefault(u => u.Id == cliams.Value);
            //ShoppingCartVM.ListCart = _unitofwork.Shopping.GetAll(sc => sc.ApplicationUserId == cliams.Value, includeProperties: "Product").Where(x => ids.Contains(x.Id.ToString()));
            ShoppingCartVM.orderHeader.ApplicationUser = _unitofwork.ApplicationUser.FirstOrDefault(u => u.Id == cliams.Value);
